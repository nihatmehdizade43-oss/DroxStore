...
async function handleGoogleLogin() {
  ...
  try {
    ...
    // Google girişi için popup yerine yönlendirme de kullanılabilir ama popup daha profesyoneldir.
    // Firebase Console'da Google Auth'un aktif olduğundan ve alan adının (localhost/render) ekli olduğundan emin olun.
    ...
    if (err.code === 'auth/popup-blocked') {
      showToast('Hata: Tarayıcı popup penceresini engelledi. Lütfen izin verin.');
    } else if (err.code === 'auth/cancelled-popup-request') {
      // ignore
    } else {
      showToast('Google Girişi başarısız: ' + err.message);
    }
  } finally {
    btn.disabled = false; 
    btn.style.opacity = '1';
